...
var App = function () {
    ...
    var handleSidebarToggler = function () {
        ...
        $('.page-sidebar .sidebar-search').on('keypress', 'input[type="text"]', function (e) {
            if (e.which == 13) {
                e.preventDefault();
        ...
        var handleSidebarFilter = function () {
            var keyword = $('.page-sidebar .sidebar-search input[type="text"]').val();
            if (keyword.length < 1) {
                $('.page-sidebar-menu li:not(.active) .sub-menu').hide();
                $('.page-sidebar-menu li:not(.active) .arrow').removeClass('open');
                $('.page-sidebar-menu li a').removeAttr("style");
                return;
            };
            $('.page-sidebar-menu .sub-menu').show();
            $('.page-sidebar-menu > li > a').hide();
            $('.page-sidebar-menu .sub-menu a').each(function (index, value) {
                if ($(this).text().toLowerCase().indexOf(keyword) > -1) { $(this).show(); } else { $(this).hide(); }
            });
            handleSidebarAndContentHeight();
        };

        // handle the search query filter menu 
        $('.page-sidebar .sidebar-search').on('keyup', 'input[type="text"]', function (e) {
            handleSidebarFilter();
        });
...
jQuery(document).ready(function () {
    ...
    var data_classfield = null;
    
    if (typeof CKFinder != 'undefined') {
        $(".upload-ckfinder").on("click", function () {
            data_classfield = $(this).attr('data-resultfield');
            var data_folderpath = $(this).attr("data-folderpath");
            var finder = new CKFinder();
            finder.removePlugins = 'help,basket,flashupload';
            finder.sidebarWidth = '0';

            if (data_folderpath != null) {
                if (data_folderpath[0] == '/') { data_folderpath = data_folderpath.slice(1, data_folderpath.length); }
                if (data_folderpath[data_folderpath.length - 1] != '/') { data_folderpath += "/";}
                finder.startupPath = "Images:/" + data_folderpath;
            }
            if (data_classfield!= null) { finder.selectActionFunction = SetFileField; }

            finder.popup();
        });
        function SetFileField(fileUrl) {
            jQuery(data_classfield).val(fileUrl);
        }
    }

    Recursion_tbMenu($('li.ite'));
    
});

function Recursion_tbMenu($ite) {
    if ($ite.length == 0) { return;}
    if (!$ite.parent().hasClass("page-sidebar-menu"))
    {
        $ite.addClass("active");
        $ite.find('a').eq(0).find('.arrow').addClass("open");
        $ite.parent().show();
        Recursion_tbMenu($ite.parent().parent());
    }        
    else
    {
        $ite.addClass("active");
        $ite.find('a').eq(0).append("<span class='selected'></span>");
        $ite.find('a').eq(0).find('.arrow').addClass("open");
        return;
    }
};
