...
namespace DreamCMS.Areas.Admin.Controllers
{
    public class OrdersController : Controller
    {
        ...
        {
            return View(db.Orders.OrderBy(x=>x.IsSeen).ThenByDescending(x=>x.OrderId).ToList());
        }
        ...
        //public ActionResult Create([Bind(Include = "OrderId,Name,SDT,Email,Adress,Date,Des")] Order order)
        //{
        ...
        {
            var item = db.OrderDetails.Where(p => p.OrderId == id).ToList();
            db.OrderDetails.RemoveRange(item);
            db.SaveChanges();
            Order order = db.Orders.Find(id);
